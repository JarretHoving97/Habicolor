    ...
    var body: some View {
        WithViewStore(self.store, observe: {$0}) { viewStore in
            VStack {
                VStack {
                    // TODO: Translations
                    DatePicker(trans("add_passed_day_view"), selection: viewStore.binding(
                        get: \.date,
                        send: HabitLogDateFeature.Action.setLogDate
                    ), in: ...(Date().adding(-1, .day) ?? Date().startOfDay), displayedComponents: .date)
                    ...
                    Button(action: {
                        ...
                        ButtonView(title: .transStandards(for: .defaultAddLabel))
                    })
